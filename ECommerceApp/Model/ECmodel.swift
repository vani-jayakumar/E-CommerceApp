...
fileprivate let urlString = "https://64bfc2a60d8e251fd111630f.mockapi.io/api/Todo"

class ECmodel {
    
    func fetchData(callBack: @escaping ([Section]?) -> Void) {
        
        if let data =  DBManager.shared.fetchCachedResponse(for: urlString) {
            if let sections = try? JSONDecoder().decode([Section].self, from: data) {
                callBack(sections)
            }
        }
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                DBManager.shared.cacheResponse(for: urlString, jsonData: data)
